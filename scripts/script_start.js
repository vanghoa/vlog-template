...
if (isSafari) {
  alert(
    "Hello ^^ If you are using Safari or browsing on IOS, the website may not display the videos and the layout correctly!"
  );
}
...
async function grow_petals(wait) {
  ...
  for (let clone of arrpetal) {
    if (wait) {
      clone.addEventListener("transitionend", clone_remap);
    } else {
      clone.classList.add("notransition");
    }
    clone.classList.add("show");
    if (wait) {
      await timeout(100);
    } else {
      clone_remap({ target: clone });
    }
  }
  ...
  function clone_remap(e) {
    ...
    clone.style.backgroundPosition = `calc(50% + ${
      width / 2 - (rect.left + rect.width / 2) + xx
    }px) calc(50% + ${height / 2 - (rect.top + rect.height / 2) + yy}px)`;
...
function closenav() {
  ...
  for (let item of arr) {
    ...
    if ((left < xcenl && left_ > xcenl) || (left < xcenr && left_ > xcenr)) {
      item.classList.add("transition_wh");
      let sz1 = left_ - xcenl;
      let sz2 = xcenr - left;
    ...
    if ((top < ycent && top_ > ycent) || (top < ycenb && top_ > ycenb)) {
      item.classList.add("transition_wh");
      let sz1 = top_ - ycent;
      let sz2 = ycenb - top;
      let szoff = sz1 < sz2 ? sz1 : sz2;
...
function keynote_nav(next) {
  ...
  if (cur == subdata.length - 1) {
    let a = $create("a");
    a.textContent = a.href = urlarrtong[week.innerText].vlog;
    a.target = "_blank";
